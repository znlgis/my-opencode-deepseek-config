...
| Agent | 模型 | 作用 |
| --- | --- | --- |
| `orchestrator` | v4-pro | 默认入口，意图门控（17 种模式）、Task Categories（6 类）、委托决策表、10 条纪律规则、9 条降级链 |
...
- 复杂重构（3+ 文件）用 `deepwork` 技能走审查门控流程：plan → review → implement → verify
- 定期用 `reflect` 技能回顾近期工作，发现配置优化点
