...
- 全局规则：`AGENTS.md`（含核心原则、任务拒绝契约、Token 效率、反模式、代码风格、注释纪律等）
...
2. 查看 agent 列表（`/agents` 或 @ 菜单），应能看到 `orchestrator`（默认）、`planner`、`deep-worker`、`oracle`、`reviewer` 等 11 个 Agent
...
| Agent | 模型 | 作用 |
| --- | --- | --- |
| `orchestrator` | v4-pro | 默认入口，意图门控（16 种模式）、Task Categories（6 类）、模型感知路由 |
...
| **v14 精简增效** | 复查 6 个参考项目当前 HEAD（遵循「精简优先于新增」）：**AGENTS.md** Skills 列表压缩为一行式（净减 ~15 行），Self-Verification 融入验证计划概念（先规划验证再动手）；**reviewer.md** Model Leverage 去重——拒绝准则细节改为引用 code-review 技能（净减 ~7 行）；**deep-worker.md** Phase 0/Step 4 去重——Todo 管理和自检步骤改为引用 AGENTS.md（净减 ~8 行）；**opencode.json** 新增 `/explore` 命令（spec-workflow explore 动作，提案前探索阶段，借鉴 OpenSpec `/opsx:explore`）；**gh-skill** 补充 cli/cli 官方 agent skill 引用（`gh skill install cli/cli gh`）；命令 24→25；README 与配置全面同步 |
| **v15 契约增效** | 复查 6 个参考项目当前 HEAD（零新增 agent/技能/命令，精简优先）：确认 gh-cli 已含 `gh repo read-file`/`read-dir`/`gh skill list`（cli/cli v2.96）、spec-workflow 已含 `update` 动作（OpenSpec v1.6）、gh 版本引用 v2.96 为当前最新——无需再动；借鉴 [oh-my-opencode-slim](https://github.com/alvinunreal/oh-my-opencode-slim) PR #842 的 **task rejection contract**——AGENTS.md 新增「任务拒绝契约」章节（越界/缺上下文/需升级时返回纯文本拒绝而非半成品，集中式定义、不逐 agent 复制，省 token）；借鉴其 PR #840 的**后台子 agent 错误核查**——orchestrator 后台调度纪律补充「合成前逐个核查失败、勿默认成功」；借鉴 [deepreview](https://github.com/mechanai/deepreview) PR #38 的 **novelty 收敛**——code-review 审查→修复循环退出条件改为 NEW/RECURRING/REGRESSION 分类（行为等价、更清晰）；确认 anomalyco `customize-opencode` 内置技能仍为最低优先兜底、复数键 schema 仍不适用本仓库 |
| **v16 Agent 清理 + 新能力** | Phase 1: Agent 清理——11 个 agent 全部移除神话名称、orchestrator 3 张路由表合并为 1 张统一表、移除冗余 AGENTS.md 引用、XML 输出格式替换为扁平 Markdown、Stats 列压缩为 tier 标签；Phase 2: gh-cli v2.96.0 更新——Issues 2.0（types、parent/child、blocking）、Discussions 命令、repo read-file/read-dir、agent skill 管理、agent 检测；Phase 3: code-review 增强——对抗性自检、项目上下文校准、确定性 finding ID、文档漂移批处理、审查→修复循环先验 finding 上下文、PR 三级分类定位；Phase 4: 新增 `verification-planning` 技能——实现前规划最窄验证路径；Phase 5: AGENTS.md 优化——新增 Model Tier Reference 集中章节、压缩 Skills 列表、挂接 verification-planning、压缩冗长段落；Phase 6: opencode.json——所有 flash agent 补 fallback 模型条目、新增 `/verify-plan` 命令；当前规模：11 agent / 16 技能 / 26 命令 |
| **v17 精准增强** | 复查 5 个参考项目最新 HEAD，minimal-pick 轻量级最优概念（零新增 agent/技能/命令）：**spec-workflow** 借鉴 OpenSpec v1.6 增加 verify 动作（archive 前三步校验：tasks 完整性 + spec 覆盖 + 一致性）与「更新 vs 新建」决策框架、design 模板增加 Open Questions 章节（可推迟的已知未知）；**reviewer** 借鉴 deepreview 增加一行项目上下文自动校准（package.json 版本 / 部署模式 / 仓库可见性，消除最常见严重度虚高）；**gh-cli** 时效刷新——补充 agent secret（`--agent` flag v2.93）、`gh skill` 扩展参数（`--allow-hidden-dirs`/`--upstream`/`--all`）、遥测状态说明；**AGENTS.md** 新增 Evidence Discipline 章节（完成前必须给出可验证证据，借鉴 oh-my-openagent「no evidence == no QA」）；agent 文件 11 个全部确认无冗余（AGENTS.md 引用均为正确委托模式）；README 同步更新安装部署与使用指南章节 |
...
这种模式下你不需要关心 Agent 怎么选、模型怎么分——Orchestrator 内置的意图门控（16 种模式）和模型感知路由会自动做出 cost-aware 的决策。
...
| 兜底 / 杂项任务 | `/generalist` | 直达 `generalist`，灵活处理 |
