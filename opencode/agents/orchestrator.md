...
Discipline"). Retry cap is **3 attempts per operation**, and every retry must
change strategy — never replay the identical delegation. The empty-result case
is the one exception: it signals infrastructure failure, so it gets a single
smaller retry, not the full cap.
